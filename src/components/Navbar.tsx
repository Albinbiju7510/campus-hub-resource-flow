...
import { Menu, X, User, Bell, Award, LogOut, ShieldAlert, ShoppingBag } from 'lucide-react';
...
const Navbar = () => {
  ...
  useEffect(() => {
    if (isAuthenticated && user && !localStorage.getItem(`welcome-shown-${user.id}`)) {
      ...
      <nav className={`${isAuthenticated ? 'bg-white' : 'bg-gradient-to-r from-blue-50 to-indigo-50'} shadow-sm sticky top-0 z-50`}>
          ...
          <div className="flex justify-between h-16">
            <div className="flex items-center">
              ...
              <div className="hidden sm:ml-10 sm:flex sm:space-x-8">
                ...
                {isAuthenticated ? (
                  <>
                    <Link to="/store" className={`${isActive('/store')} border-b-2 px-1 pt-1 pb-3 text-sm font-medium`}>
                      Store
                    </Link>
        ...
        <div className={`${isMenuOpen ? 'block' : 'hidden'} sm:hidden`}>
          <div className="pt-2 pb-3 space-y-1">
            ...
            {isAuthenticated && (
              <>
                <Link to="/store" className={`${isActive('/store') ? 'bg-campus-light border-campus-primary text-campus-primary' : 'border-transparent text-gray-700'} block pl-3 pr-4 py-2 border-l-4 text-base font-medium`}>
                  Store
                </Link>
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="text-2xl text-center">
              Welcome to CampusHub, {user?.name}!
            </DialogTitle>
            <DialogDescription className="text-center">
              ...
              <p className="mt-4">Thank you for joining CampusHub. Start exploring resources, earning points, and connecting with the campus community!</p>
            </DialogDescription>
          </DialogHeader>
        </DialogContent>
      </Dialog>
